      ...
      <div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <a-card
              v-if="tripPlan.rag_contexts?.length"
              title="📚 本地攻略参考"
              :bordered="false"
              class="rag-card"
            >
              <div class="rag-meta">
                <a-tag color="purple">
                  {{ tripPlan.rag_retrieval_status === 'hit' ? 'RAG 命中' : '本地知识库' }}
                </a-tag>
                <span class="rag-count">{{ tripPlan.rag_contexts.length }} 条参考片段</span>
              </div>
              <a-collapse :bordered="false" accordion>
                <a-collapse-panel
                  v-for="(context, index) in tripPlan.rag_contexts.slice(0, 3)"
                  :key="`rag-${index}`"
                >
                  <template #header>
                    攻略片段 {{ index + 1 }}
